...
export default {
	...
	data() {
		return {
			...
			res_cir: ['가슴이 아파요', '숨이 잘 안 쉬어져요', '기침이 나요', '콧물이 나요', '해당없음'],
			gynecology: ['질에서 피가 나와요', '냉이 나와요', '월경이 불규칙해요', '임신한 것 같아요', '해당없음'],
			whole_body: ['열이 나요', '몸이 으슬으슬 떨려요', '피곤해요', '최근 체중이 변했어요', '밤에 식은 땀이 나요', '잠자기가 어려워요', '머리가 아파요', '해당없음'],
